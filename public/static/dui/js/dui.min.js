(function (global, factory) {
  ...
  Dui.amd = {}; // 增加<script src="__DUI__/js/dui.min.js" data-base="__ADMIN_JS__/" data-plugin="admin"></script>的用法
  ...
  function getProps(el, propconfig) {
  ...
  function enter() {
    ...
    var toClass = data.name + '-enter-to'; // 设置为不显示

    setStyle(el, 'display', 'none'); // 4个生命周期钩子函数
  ...
  function leave() {
    var that = this,
        data = that.config,
        el = that.elem; // call leave callback now
    ...
    var toClass = data.name + '-leave-to'; // 设置为不显示

    setStyle(el, 'display', ''); // 4个生命周期钩子函数
  ...
  Class.prototype = Class.fn = {
    init: function init(elem, options) {
      ...
      var data = that.config = options; //设置状态
  ...
  function scrollbar(el, options) {
    ...
    that.data = extend(true, {}, options); // 创建scroll
  ...
  function off$1(el, event, fn) {
    ...
    if (el.events && el.events[event]) {
      ...
      el.events[event] = el.events[event].filter(function (d) {
        return d;
      });
