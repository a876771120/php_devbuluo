(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('jquery'), require('form')) :
    typeof define === 'function' && define.amd ? define('tree', ['jquery', 'form'], factory) :
    (global = global || self, global.tree = factory(global.jQuery, global.form));
}(this, function ($, form) { 'use strict';
    ...
    var Class = function Class(options) {
      ...
      that.options = options; // 用来装树形结构的容器

      that.el = $(config.el); // 根据数据源的类型加载数据

      if ($.isArray(config.data)) {
        that.data = $.extend(true, [], config.data);
        that.render();
      } else if (config.data && config.data.url) ; else {
        console.error('数据源类型错误！');
        return;
      }
    },
        TREE_NODE = 'dui-tree-node',
        EXPAND_ICON = 'dui-tree-node__expand-icon',
        TREE_CHILD = 'dui-tree-node__children',
        TREE_CONTENT = 'dui-tree-node__content',
    ...
    Class.prototype.render = function () {
      var that = this,
          data = that.data,
          config = that.config,
          view = that.view(data, 1);
      that.el.html('').addClass('dui-tree');
      that.inner = $(view);
      that.el.append(that.inner); // 渲染页面

      form.render(that.el[0], 'checkbox'); // 设置当前选中

      that.checkeds = $.extend(true, [], config.checkeds); // 层级数据转列表数据

      that.dataList = that.layerToList(data); // 设置事件
    ...
    Class.prototype.layerToList = function (data, pid) {
      ...
      var digui = function digui(thisData, thisPid) {
        var res = {};
    ...
    Class.prototype.view = function (data, level) {
      ...
      $.each(data, function (i, item) {
        ...
        }() + '">', '<div class="dui-tree-node__content" data-key="' + item[config.idName] + '" style="padding-left:' + (level - 1) * config.indent + 'px">', '<i class="dui-tree-node__expand-icon dui-icon-arrow-right' + function () {
        ...
        }(), '<span class="dui-tree-node__label">' + item.label + '</span>', '</div>', function () {
          if (item[config.childName] && $.isArray(item[config.childName])) {
    ...
    Class.prototype.evens = function () {
      var that = this,
          config = that.config; //设置点击事件

      that.el.find('.' + TREE_NODE).on('click', function (e) {
        e.stopPropagation();
        var othis = $(this),
            isExpand = othis.hasClass('is-expanded'),
      ...
      .find('.dui-checkbox').on('click', function (e) {
        e.stopPropagation();
        var othis = $(this),
            treeNode = othis.parent(),
            id = treeNode.data('key'),
            thisData = that.dataList[id],
            childs = thisData[config.childName] || [],
            checkbox = othis.find('input[dui-checkbox]'),
            checked = checkbox[0].checked,
            checkedIds = [].concat(getDownIds(childs)).concat(getUpIds(thisData)); // 获取所有的下级id
        ...
        function getUpIds(thisData) {
          ...
          if (item) {
            ...
            if (checked == false) {
              // 获取当前item的子数据
              var tempChild = $.extend(true, {}, item[config.childName]); // 删除当前子节点
            ...
            if (!isInArray) {
              res.push(item[config.idName]); // 如果有父节点
