(function (global, factory) {
  ...
  typeof define === 'function' && define.amd ? define('popup', ['jquery'], factory) :
  (global = global || self, global.popup = factory(global.jQuery));
}(this, function ($) { 'use strict';

  $ = $ && $.hasOwnProperty('default') ? $['default'] : $;

  var popup = function popup(type, options) {
    ...
    $.each(pups, function (i, item) {
      ...
      resetNotification = function resetNotification(x, y) {
    var pups = [],
        currenheight = 0,
        type = 'notify';
    $.each(allPopup, function (i, item) {
    ...
    var that = this,
      ...
      offset: 20
        ...
        template = that.template = ['<div class="dui-message' + (config.type && !config.iconClass ? ' dui-message--' + config.type : ''), config.center ? ' is-center' : '', config.showClose ? ' is-closable' : '', '" style="" id="' + id + '">', function () {
      if (config.iconClass) {
        return '<i class="' + config.iconClass + '"></i>';
      } else {
        return '<i class="dui-message__icon dui-icon-' + config.type + '"></i>';
      }
    }(), '<p class="dui-message__content">' + config.message + '</p>', config.showClose ? '<i class="dui-message__closeBtn dui-icon-close"></i>' : '', '</div>'].join(''),
        dom = that.dom = $(template),
        getTop = function getTop(type, cur) {
      var num = 0,
          allHeight = 0;
    ...
    var offsetHeight = that.offsetHeight = dom.outerHeight(); // 设置z-index
    ...
    if (position.num > 0) dom.css('top', position.top); // 设置事件

    dom.hover(clearTimer, startTimer); // 设置点击事件
    ...
    var that = this,
      ...
      offset: 20
        ...
        template = that.template = ['<div class="dui-notification ' + horizontalClass + (config.customClass ? ' ' + config.customClass : '') + '" id="' + id + '">', config.type || config.iconClass ? '<i class="' + (config.type ? typeClass : config.iconClass) + '"></i>' : '', '<div class="dui-notification__group' + (config.type || config.iconClass ? ' is-with-icon' : '') + '">', '<h2 class="dui-notification__title">' + config.title + '</h2>', config.message ? '<div class="dui-notification__content"><p>' + config.message : '', config.message ? '</p></div>' : '', config.showClose ? '<div class="dui-notification__closeBtn dui-icon-close"></div>' : '', '</div>', '</div>'].join(''),
        dom = that.dom = $(template),
        getPosition = function getPosition(cur) {
      var num = 0,
          allHeight = 0,
          type = 'notify';
      $.each(allPopup, function (id, item) {
    ...
    var position = getPosition(that); // 设置距离

    dom.css(verticalProperty, position.px); // 设置事件
    ...
    var that = this,
      ...
      var res = [],
          thisBtn = $.extend(true, {}, config).btns,
          btns = config.btnAngin == 'right' ? thisBtn.reverse() : thisBtn;
      $.each(btns, function (i, btn) {
        var btnStr = ['<button type="button" class="dui-button dui-button--' + (config.btnAngin == 'right' && i == btns.length - 1 || config.btnAngin != 'right' && i == 0 ? 'primary' : 'default') + ' dui-button--small">', '<span>' + btn + '</span>', '</button>'].join('');
        res.push(btnStr);
      });
      return res.join('');
    }(),
        template = ['<div class="dui-popup' + (config.customClass ? ' ' + config.customClass : '') + '">', '<div class="dui-popup__header">', '<div class="dui-popup__title">', '<span style="display:block">' + config.title + '</span>', '</div>', config.showClose ? '<button type="button" class="dui-popup__headerbtn"><i class="dui-popup__close dui-icon-close"></i></button>' : '', '</div>', '<div class="dui-popup__content"><span>' + config.content + '</span></div>', config.showFooter ? '<div class="dui-popup__btns"' + (config.btnAngin != 'right' ? 'style=" text-align:' + config.btnAngin + ';"' : '') + '>' + btns + '</div>' : '', '</div>', config.modal ? '<div class="dui-modal"></div>' : ''].join(''),
        _doc = $(document),
        win = $(window),
        dom = that.dom = $(template),
        ...
        clickbtns = footer.find('button'); // 给元素设置z-index
    ...
    setResize = function setResize() {
      ...
      if (height !== 'auto') {
        $(dom[0]).css('height', height); //设置body的高度
        //获取headerHeight
    ...
    var move = {},
        docmousemove = function docmousemove(e) {
      if (move.moveStart) {
        e.preventDefault();
        ...
        move.stY = fixed ? 0 : win.scrollTop(); //控制元素不被拖出窗口外
        ...
        docmouseup = function docmouseup(e) {
    ...
    _doc.on('mousemove', docmousemove).on('mouseup', docmouseup); // 设置点击按钮事件
    ...
    var that = this,
      ...
      customClass: ''
        ...
        template = ['<div class="dui-loading-mask" ' + (config.background ? ' style="background-color:' + config.background + '"' : '') + '>', '<div class="dui-loading-spinner">', config.spinner ? '<i class="' + config.spinner + '"></i>' : '<svg viewBox="25 25 50 50" class="circular"><circle cx="50" cy="50" r="20" fill="none" class="path"></circle></svg>', config.text ? '<p class="dui-loading-text">' + config.text + '</p>' : '', '</div>', '</div>'].join(''); // 如果没有target则直接返回

    if (!config.target) return;
    var target = that.target = $(config.target); // 需要插入的元素

    var showDom = that.showDom = $(template); // 设置元素为none

    showDom.css('display', 'none'); // 设置过渡

    var transition = that.transition = dui.transition(showDom[0], {
      name: 'dui-loading-fade',
      afterLeave: function afterLeave(el) {
        // 给target添加class
        target.removeClass('dui-loading-parent--relative'); // 如果是lock

        target.removeClass('dui-laoding-parent--hidden'); // 移除显示元素
  ...
  loading.prototype.show = function () {
    var that = this,
        config = that.config; // 给target添加class

    that.target.addClass('dui-loading-parent--relative'); // 如果是lock

    config.lock && that.target.addClass('dui-laoding-parent--hidden'); // 添加元素

    that.target.append(that.showDom); // 显示元素
  ...
  popup.msgbox = function (options) {
    var type = options.type,
        ...
        content = [type ? '<div class="dui-popup__status dui-icon-' + type + '"></div>' : '', '<div class="dui-popup__message"><p>' + message + '</p></div>'].join('');
    options = $.extend(true, options, {
      width: '420px',
      content: content
    });
