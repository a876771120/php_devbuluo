(function (global, factory) {
    ...
    typeof define === 'function' && define.amd ? define('pagination', ['jquery', 'form'], factory) :
    (global = global || self, global.pagination = factory(global.jQuery, global.form));
    ...
    Class.prototype.render = function () {
      ...
      that.dom = $(views);
      that.parent.html(that.dom); //渲染

      form.render(that.dom[0]); //设置事件
    ...
    Class.prototype.view = function () {
      ...
      res['total'] = '<span class="dui-pagination__total">共 ' + config.total + ' 条</span>', res['sizes'] = ['<span class="dui-pagination__sizes">', function () {
        ...
        return html + '</select>';
      }(), '</span>'].join(''), res['prev'] = '<button type="button" class="btn-prev"' + function () {
        if (config.current == 1) {
          return 'disabled="disabled"';
        }
      }() + '>' + config.prev + '</button>', res['pager'] = ['<ul class="dui-pager">', function () {
        var pagerDom = []; //数据量为0时，不输出页码
        ...
        var index = config.Allpage > pagerNum ? Math.ceil((config.current + (pagerNum > 1 ? 1 : 0)) / (pagerNum > 0 ? pagerNum : 1)) : 1; //首页
      ...
      }(), '</ul>'].join(''), res['next'] = '<button type="button" class="btn-next"' + function () {
        if (config.current == config.Allpage) {
          return 'disabled="disabled"';
        }
      }() + '>' + config.next + '</button>', res['jumper'] = ['<span class="dui-pagination__jump">前往<div class="dui-input dui-pagination__editor is-in-pagination">', '<input type="number" class="dui-input__inner" name="pager-jump" value="' + config.current + '">', '</div>页</span>'].join('');
    ...
    Class.prototype.setEvent = function () {
      var that = this,
          config = that.config; //按钮切换事件

      that.dom.on('click', '.dui-pager .number', function (e) {
        var othis = $(this),
            page = othis.data('page');
    ...
    Class.prototype.jump = function (page, size) {
      ...
      config.size = size;
      config.current = page; //渲染第一次

      that.render();

      if (page > config.Allpage) {
        //当当前页大于总页则赋值为最大页
        config.current = config.Allpage; //然后再渲染
