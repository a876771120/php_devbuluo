(function (global, factory) {
  ...
  function paddingH(display) {
    ...
    var e = removeChildrenAndAdd(display.measure, elt("pre", "x", "CodeMirror-measure-elem"));
  ...
  function textHeight(display) {
    ...
    if (measureText == null) {
      measureText = elt("pre", null, "CodeMirror-measure-elem"); // Measure a bunch of lines, for browsers that compute
      // fractional heights.
  ...
  function charWidth(display) {
    ...
    var pre = elt("pre", [anchor], "CodeMirror-measure-elem");
  ...
  CodeMirror.overlayMode = CodeMirror.overlayParser = function (base, overlay, combine) {
    return {
      ...
      token: function token(stream, state) {
        ...
        if (state.baseCur != null && combine) return state.baseCur + " " + state.overlayCur;else return state.overlayCur;
      },
  ...
  CodeMirror.defineMode("markdown", function (cmCfg, modeCfg) {
    ...
    var tokenTypes = {
      header: "header  ace_heading",
      code: "comment ace_support ace_function xsj_code",
      quote: "quote ace_blockquote ace_string",
      list1: "variable-2 ace_list",
      list2: "variable-3 ace_list",
      list3: "keyword ace_list",
      hr: "hr",
      image: "image ace_string",
      imageAltText: "image-alt-text",
      imageMarker: "image-marker",
      formatting: "formatting",
      linkInline: "link ace_string ace_link",
      linkEmail: "link ace_string ace_link",
      linkText: "link ace_string ace_link",
      linkHref: "link string ace_underline ace_string ace_link",
      em: "em ace_string ace_emphasis",
      strong: "strong ace_string ace_strong",
    ...
    function getType(state) {
      ...
      if (state.header) {
        styles.push(tokenTypes.header, tokenTypes.header.replace(/\S+/g, function (word) {
          if (word.indexOf('ace_') == 0) {
            return word + "_" + state.header;
          }

          return word + "-" + state.header;
        }));
      }
  ...
  CodeMirror.defineMode("gfm", function (config, modeConfig) {
    ...
    var gfmOverlay = {
      ...
      token: function token(stream, state) {
        ...
        if (stream.sol() || state.ateSpace) {
          ...
          if (modeConfig.gitHubSpice !== false) {
            if (stream.match(/^(?:[a-zA-Z0-9\-_]+\/)?(?:[a-zA-Z0-9\-_]+@)?(?=.{0,6}\d)(?:[a-f0-9]{7,40}\b)/)) {
              ...
              return "link ace_underline string url ace_string ace_link";
        ...
        if (stream.match(/^==\S+==$/)) {
          stream.string = stream.string + 'aaaaa';
          return 'mark ace_mark';
        }
  ...
  InlineLexer.prototype.output = function (src) {
    ...
    while (src) {
      ...
      if (cap = this.rules.text.exec(src)) {
        ...
        if (this.inRawBlock) {
          out += this.renderer.text(this.options.sanitize ? this.options.sanitizer ? this.options.sanitizer(cap[0]) : escape$1(cap[0]) : cap[0]);
        } else {
  ...
  TextRenderer.prototype.strong = TextRenderer.prototype.mark = TextRenderer.prototype.em = TextRenderer.prototype.codespan = TextRenderer.prototype.del = TextRenderer.prototype.text = function (text) {
    return text;
  };
  ...
  function escape$1(html, encode) {
    if (encode) {
      if (escape$1.escapeTest.test(html)) {
        return html.replace(escape$1.escapeReplace, function (ch) {
          return escape$1.replacements[ch];
        });
      }
    } else {
      if (escape$1.escapeTestNoEncode.test(html)) {
        return html.replace(escape$1.escapeReplaceNoEncode, function (ch) {
          return escape$1.replacements[ch];
        });
      }
    }
  ...
  function cleanUrl(sanitize, base, href) {
    if (sanitize) {
      try {
        var prot = decodeURIComponent(unescape$1(href)).replace(/[^\w:]/g, '').toLowerCase();
      } catch (e) {
        return null;
      }
  ...
  function mergeStreams(original, highlighted, value) {
    ...
    function open(node) {
      function attr_str(a) {
        return ' ' + a.nodeName + '="' + escape$2(a.value).replace('"', '&quot;') + '"';
      }
    ...
    while (original.length || highlighted.length) {
      ...
      result += escape$2(value.substring(processed, stream[0].offset));
    ...
    return result + escape$2(value.substr(processed));
  }
  /* Initialization */
  ...
  */
  ...
  function highlight(name, value, ignore_illegals, continuation) {
    ...
    function processKeywords() {
      ...
      if (!top.keywords) return escape$2(mode_buffer);
      result = '';
      last_index = 0;
      ...
      while (match) {
        result += escape$2(mode_buffer.substring(last_index, match.index));
        keyword_match = keywordMatch(top, match);

        if (keyword_match) {
          relevance += keyword_match[1];
          result += buildSpan(keyword_match[0], escape$2(match[0]));
        } else {
          result += escape$2(match[0]);
        }
      ...
      return result + escape$2(mode_buffer.substr(last_index));
    }
  ...
  */
  ...
  var lexer = new marked.Lexer(); // 源代码是merge了新的block rules，所以需要在实例上再添加一次
  ...
  Object.assign(lexer.rules, customRules); // 重写词法解析器
  ...
  marked.Lexer.prototype.token = function (src, top) {
    ...
    while (src) {
      ...
      if (cap = this.rules.blockquote.exec(src)) {
        src = src.substring(cap[0].length);
        var Identification = /\> {0,1}\[(\S+)\]/.exec(cap[0]);
        this.tokens.push({
          type: 'blockquote_start'
        });
        cap = cap[0].replace(/^ *> ?/gm, ''); // Pass `top` to keep the current
        ...
        this.tokens.push({
          type: 'blockquote_end',
          Identification: Identification ? Identification[1] : null
        });
        continue;
      } // list
      ...
      case 'blockquote_start':
        {
          ...
          return this.renderer.blockquote(body, this.token.Identification);
        }
  ...
  var renderer = new marked.Renderer(); // code高亮

  renderer.code = function (code, language, isEscaped) {
    var isMarkup = language === 'markup';
    var hled;
    ...
    return "<pre class=\"hljs\"><code class=\"".concat(language, "\">").concat(hled, "</code></pre>");
  };

  renderer.listitem = function (body, task, checked) {
    var className = '';
    ...
    return "<li class=\"".concat(className, "\">").concat(body, "</li>\n");
  };
  ...
  mdEditor.$marked = marked; // 工具条点击的回调功能方法配置
  ...
  mdEditor.className = {
    toolbarContainer: mdEditor.classPrefix + '__toolbar',
    //工具条容器
    toolbarLeftContainer: mdEditor.classPrefix + '__toolbar-left',
    //工具栏左侧样式
    toolbarRightContainer: mdEditor.classPrefix + '__toolbar-right',
    //工具栏右侧样式
    toolbarDivider: mdEditor.classPrefix + '__toolbar-divider',
    toolbarOperation: mdEditor.classPrefix + '__toolbar-op',
    //操作按钮
    toolbarTip: mdEditor.classPrefix + '__toolbar-tip',
    //操作提示
    panelContainer: mdEditor.classPrefix + '__panel',
    //面板容器
  ...
  mdEditor.toolbarHasStatusIcons = ['preview', 'fullscreen', 'readmodel', 'fullscreen', 'subfield', 'catalog']; // 编辑器默认配置信息
  ...
  mdEditor.defaults = {
    ...
    marked: {},
    toolBars: {
      left: ['bold', 'italic', 'header', 'divider', 'strikethrough', 'mark', 'alignleft', 'aligncenter', 'alignright', 'divider', 'quote', 'list-ol', 'list-ul', 'divider', 'link', 'imagelink', 'code', 'table', 'divider', 'undo', 'redo', 'trash', 'save'],
      //左侧按钮
  ...
  function Class(options) {
    var that = this,
        config = that.config = $.extend(true, mdEditor.defaults, options);
    if (!$(config.el)[0]) throw new Error('Initialization failed because there was no editor container'); // 编辑器容器

    var el = that.el = $(config.el); // 如果容器是textarea，则需要生成容器

    if (!el.is("textarea")) throw new Error('Initialization failed ，Element only is textarea'); // 设置文本域

    var textarea = that.textarea = el; // 是否是渲染过的组件

    var isRender = el.parents('.' + mdEditor.classPrefix); // 判断是否已经渲染过了

    if (isRender[0]) {
      // 销毁
      that.cm.toTextarea(); // 移动元素到之前节点

      isRender.before(textarea); // 还原textarea的样式
  ...
  Class.prototype = {
    ...
    setLayout: function setLayout() {
      var that = this,
          config = that.config; // 获取当前textarea宽度
      ...
      var mdeditorContainer = that.mdeditorContainer = $('<div class="' + mdEditor.classPrefix + '"></div>'); // 工具条容器

      var toolbarContainer = that.toolbarContainer = $('<div class="' + mdEditor.className.toolbarContainer + '"></div>'); // 面板容器

      var panelContainer = that.panelContainer = $(['<div class="' + mdEditor.className.panelContainer + '">', '<div class="' + mdEditor.className.editorContainer + '"></div>', '<div class="' + mdEditor.className.previewContainer + '"></div>', '</div>'].join('')); // codemirror容器

      var editorContainer = that.editorContainer = panelContainer.find('.' + mdEditor.className.editorContainer); // 视图容器

      var previewContainer = that.previewContainer = panelContainer.find('.' + mdEditor.className.previewContainer); // 设置mdEditor的宽高
      ...
      that.el.after(mdeditorContainer); // 添加工具条容器

      mdeditorContainer.append(toolbarContainer); // 添加面板容器

      mdeditorContainer.append(panelContainer); // 把文本域移动到codemirror容器

      editorContainer.append(that.el); // 设置textarea的样式
     ...
     * @returns Class
     */
    setToolbarView: function setToolbarView() {
      var that = this,
          config = that.config;
      var btnList = {
      ...
      var IconStatusManager = that.IconStatusManager; // 语言

      var tips = config.langs.toolbar.tip; // 语言

      var insets = config.langs.toolbar.inset; // 图标

      var icons = config.toolbarIcons; // 使用文字代替图标

      var iconTexts = config.toolbarIconTexts; // 有状态的工具按钮

      var hasStatus = that.hasStatus = mdEditor.toolbarHasStatusIcons.concat(config.toolbarHasStatusIcons); // 工具栏左侧容器

      var toolbarLeftContainer = $('<ul class="' + mdEditor.className.toolbarLeftContainer + '"></ul>'); // 工具栏右侧容器

      var toolbarRightContainer = $('<ul class="' + mdEditor.className.toolbarRightContainer + '"></ul>'); // 添加工具栏按钮
      ...
      $.each(btnList, function (algin, list) {
        $.each(list, function (key, item) {
          ...
          if (key == 'divider') {
            ...
            btn = $('<li class="' + mdEditor.className.toolbarOperation + (info.icon ? ' ' + info.icon : '') + (IconStatusManager[key] ? ' is-selected' : '') + '">' + (info.icon ? '' : info.iconText) + '<span class="' + mdEditor.className.toolbarTip + '">' + (typeof info.tip === "string" ? info.tip.split(' ')[0] : '') + '</span>' + '</li>');
            btn[0].key = key;
            btn[0].title = info.tip || ''; // 设置工具栏回调事件
          }
      ...
      that.toolbarContainer.append(toolbarLeftContainer).append(toolbarRightContainer); // 设置工具栏处理事件
     ...
     * @returns Class
     */
    bindDefaultToolbarHandler: function bindDefaultToolbarHandler() {
      var that = this,
          config = that.config; // 事件处理的配置

      var callBackConfig = $.extend(true, mdEditor.callBackConfig, config.callBackConfig); // 当前实例事件处理容器

      var toolbarHandlers = that.toolbarHandlers = {}; // 工具栏的插入语言

      var insets = config.langs.toolbar.inset; // 工具栏的提示语言

      var tips = config.langs.toolbar.tip; // 所有的工具栏按钮

      var allToolbarBtn = [].concat(config.toolBars.left).concat(config.toolBars.right); // 有状态的按钮

      var hasStatus = that.hasStatus; // 循环添加默认事件

      $.each(allToolbarBtn, function (i, key) {
        // 设置事件容器
        toolbarHandlers[key] = {}; // 设置点击事件和快捷键事件
        ...
        toolbarHandlers[key].click = toolbarHandlers[key].shortcutkey = function (e, cm) {
          ...
          if ($.inArray(key, hasStatus) !== -1) {
            // 获取状态管理器
            var status = that.IconStatusManager[key]; // 图标样式名

            var iconClass = config.toolbarIcons[key] && typeof config.toolbarIcons[key] === "string" ? config.toolbarIcons[key] : config.toolbarIcons[key] && typeof config.toolbarIcons[key][status ? 'off' : 'on'] === "string" ? config.toolbarIcons[key][status ? 'off' : 'on'] : ''; // 也可能是使用了文字

            var iconText = !iconClass ? config.toolbarIconTexts[key] && typeof config.toolbarIconTexts[key] === "string" ? config.toolbarIconTexts[key] : config.toolbarIconTexts[key] && typeof config.toolbarIconTexts[key][status ? 'off' : 'on'] === "string" ? config.toolbarIconTexts[key][status ? 'on' : 'off'] : '' : ''; // 获取tip

            var tip = tips[key] && typeof tips[key] === "string" ? tips[key] : tips[key] && typeof tips[key][status ? 'off' : 'on'] === "string" ? tips[key][status ? 'off' : 'on'] : ''; // 按钮的样式名称

            var btnClass = mdEditor.className.toolbarOperation + ' ' + iconClass + (status ? '' : ' is-selected'); // 当前状态为选中
            ...
            othis.toolbarContainer.find('.' + mdEditor.className.toolbarOperation).each(function (i, btn) {
              ...
              if (key == btn.key) {
                // 当前按钮
                var $btn = $(btn),
                    $tip = $btn.find('.' + mdEditor.className.toolbarTip); // 设置样式

                $btn.attr('class', btnClass).attr('title', tip).text(iconText).append($tip); // 设置编辑器样式

                othis.editorContainer.removeClass(status === true ? key + '-selected' : '').addClass(status === true ? '' : key + '-selected'); // 设置提示
          ...
          if (callBackConfig[key] && typeof callBackConfig[key] === "object") {
            ...
            if (selection) {
              ...
              end.ch = suffixLine ? suffixlen : cursor.ch + defaultText.length + prefixlen; // 添加字符串

              cm.replaceSelection(info.prefix + defaultText + info.suffix); // 设置选中
          ...
          if (key == 'fullscreen') {
            // 站位元素
            that.stanceContainer = that.stanceContainer ? that.stanceContainer : $('<div class="' + mdEditor.className.stanceContainer + '">'); // 退出全屏

            if (status === true) {
              // 还原元素
              that.stanceContainer.after(othis.mdeditorContainer); // 移除站位元素

              that.stanceContainer.remove(); // 移除样式

              othis.mdeditorContainer.removeClass('is-fullscreen');
            } //进入全屏
            else {
                // 插入站位元素
                othis.mdeditorContainer.before(that.stanceContainer); // 把节点插入到body

                $('body').append(othis.mdeditorContainer); // 然后设置样式
          ...
          if (key == 'subfield') {
            ...
            othis.toolbarContainer.find('.' + mdEditor.className.toolbarOperation).each(function (i, btn) {
              ...
              if ('preview' == btn.key) {
                // 当前按钮
                var $btn = $(btn),
                    $tip = $btn.find('.' + mdEditor.className.toolbarTip);
                var title = status === true ? tips.preview.off : tips.preview.on; // 设置当前按钮是不选中

                $btn.removeClass(status === true ? 'is-selected' : '').addClass(status === true ? '' : 'is-selected').attr('title', title); // 取消codeMirror容器的样式

                othis.editorContainer.removeClass(status === true ? 'preview-selected' : '').addClass(status === true ? '' : 'preview-selected').attr('title', title); // 设置提示
    ...
    setToolbarHandler: function setToolbarHandler() {
      var that = this,
          config = that.config; // 绑定默认的事件

      that.bindDefaultToolbarHandler(); // 获取已经绑定的事件,包含用户自定义

      var toolbarHandlers = that.toolbarHandlers = $.extend(true, that.toolbarHandlers, config.toolbarHandlers); // 工具栏容器

      var toolbarContainer = that.toolbarContainer; // 获取所有的工具栏按钮

      var toolbarBtnIcon = toolbarContainer.find('.' + mdEditor.className.toolbarOperation); // 循环绑定事件,因为需要设置样式
      // 鼠标进入
      ...
      $(toolbarBtnIcon).on(mdEditor.mouseOrTouch("click", "touchend"), function (e) {
        ...
        if (name !== "link" && name !== "image" && name !== "code-block" && name !== "subfield" && name !== "watch" && name !== "preview" && name !== "search" && name !== "fullscreen" && name !== "help") {
          that.cm.focus();
        }
      });
      return that;
     ...
     */
    getCallBackParams: function getCallBackParams() {
      var that = this,
          res = {};
    ...
    setCodemirror: function setCodemirror() {
      var that = this,
          config = that.config; // 当前codeMirror的容器

      var editorContainer = that.editorContainer; // 如果没有设置主题则设置默认

      !config.codeMirror.theme && (config.codeMirror.theme = 'default'); // 设置编辑器主题
      ...
      var codeMirrorConfig = $.extend(true, {
        ...
        //括号匹配
        indentWithTabs: true,
        //在缩进时，是否需要把 n*tab宽度个空格替换成n个tab字符
        ...
        //目前暂未可知
        highlightSelectionMatches: !config.codeMirror.matchWordHighlight ? false : {
          showToken: config.codeMirror.matchWordHighlight === "onselected" ? false : /\w/
        }
      }, config.codeMirror, {
      ...
      that.codeMirror = that.cmContainer = editorContainer.children('.CodeMirror'); // 绑定文本变化事件
     ...
     * @returns Class
     */
    bindChangeHandler: function bindChangeHandler() {
      var that = this,
          config = that.config,
          cm = that.cm;
    ...
    completed: function completed() {
      var that = this,
          config = that.config; // 保存一个代码,因为如果有初始值，则需要解析成markdown
    ...
    save: function save() {
      ...
      var renderer = new marked.Renderer(); // 自定义语法的渲染函数

      renderer.mark = function (text) {
        return "<mark>".concat(text, "</mark>");
      };

      renderer.textAlign = function (align, text) {
        return "<div class=\"hljs-".concat(align, "\">\n                        <p>").concat(text, "</p>\n                    </div>");
      };

      renderer.blockquote = function (body, Identification) {
        var content = /\ {0,1}\[\S+\](\S+)/.exec(body);
        content = content ? '<p>' + content[1] : body;
        return "<blockquote class=\"".concat(Identification && ['info', 'warning', 'success', 'danger'].indexOf(Identification) !== -1 ? Identification : 'default', "\">\n                        ").concat(content, "\n                    </blockquote>");
      }; // 配置信息
      ...
      var markedOptions = that.markedOptions = {
        ...
        breaks: true,
        pedantic: false,
        sanitize: config.htmlDecode ? false : true,
        // 关闭忽略HTML标签，即开启识别HTML标签，默认为false
        smartLists: true,
        smartypants: true
      };
      marked.setOptions(markedOptions); // 获取了html

      var markHtml = marked(cmValue); // 设置markdown数据

      that.textarea.val(cmValue); // 保存缓存数据
