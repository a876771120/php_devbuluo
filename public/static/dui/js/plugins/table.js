(function (global, factory) {
    ...
    TMPL_HEAD = function TMPL_HEAD(options) {
      ...
      '<th{{if item2.minWidth}} data-minWidth="{{item2.minWidth}}"{{/if}}{{if item2.unresize || item2.colspan>1}} unresize="{{item2.unresize}}"{{/if}}', '{{if item2.field}} data-field="{{item2.field}}"{{/if}}', ' data-key="{{item2.key}}" colspan="{{item2.colspan}}" rowspan="{{item2.rowspan}}"', ' class="{{if item2.sort && item2.colspan==1}}is-sortable{{/if}}{{if item2.type!=="normal"}} dui-table__cell-{{item2.type}}{{/if}}{{if item2.align}} is-{{item2.align}}{{/if}}">', '<div class="cell dui-table-{{if item2.colGroup}}group{{else}}{{index}}-{{item2.key}}{{/if}}">', '{{if item2.type=="checkbox"}}', '<input type="checkbox" dui-checkbox indeterminate="true"{{if checkAll}} checked="checked"{{/if}} dui-filter="checkAll">', '{{else}}', '{{item2.title}}', '{{/if}}', // 排序
      ...
      ' style="{{if height}}height:{{height}}px{{/if}}">', '<div class="dui-table__header-wrapper">', TMPL_HEAD(), '</div>', '<div class="dui-table__body-wrapper{{if initScroll}} {{initScroll}}{{/if}}" style="height:{{bodyHeight}}px;">', // 显示内容，暂不设置
      '</div>', '{{if fixedLCoumnsNum>0}}', '<div class="dui-table__fixed dui-table__fixed-left">', '<div class="dui-table__fixed-header-wrapper">', TMPL_HEAD({
        ...
        fixed: 'right'
      }), '</div>', '<div class="dui-table__fixed-body-wrapper" style="height:{{bodyHeight}} px;">', '</div>', '</div>', '{{/if}}', '{{if page.show}}', '<div class="dui-table__page">', '</div>', '{{/if}}', // 补丁
      '<div class="dui-table__fixed-right-patch"></div>', // 设置样式
