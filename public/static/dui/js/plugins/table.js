(function (global, factory) {
    ...
    popup = popup && popup.hasOwnProperty('default') ? popup['default'] : popup;
    pagination = pagination && pagination.hasOwnProperty('default') ? pagination['default'] : pagination;
    ...
    condition = {
      ...
      'enum': ['eq', 'neq']
        ...
        // 常量定义
    ELEM = '.dui-table',
        TABLEBOX = '.dui-table__box',
        HEADER = '.dui-table__header-wrapper',
        BODYER = '.dui-table__body-wrapper',
        FIXED = '.dui-table__fixed',
        FIXED_LEFT = '.dui-table__fixed-left',
        FIXED_RIGHT = '.dui-table__fixed-right',
        PAGE = '.dui-table__page',
        CONDITION = '.dui-table__condition',
        PATCH = '.dui-table__fixed-right-patch',
        FIXED_WRAP = '.dui-table__fixed-body-wrapper',
        TABLEBODY = '.dui-table__body',
        ROWHOVER = '.dui-table__body tr',
        FIXED_HEAD = '.dui-table__fixed-header-wrapper',
        TABLEHEADER = '.dui-table__header',
        HEADER_TH = TABLEHEADER + ' th',
        HEADER_SORT = '.caret-wrapper',
        TREETABLE_EXPAND = 'dui-table__expand-icon',
        TREETABLE_EXPANDED = 'dui-table__expand-icon--expanded',
        // 列模板
    TMPL_HEAD = function TMPL_HEAD(options) {
      options = options || {};
      return ['<table cellspacing="0" cellpadding="0" border="0" class="dui-table__header">', '<thead>', '{{each columns  item1 i1}}', '<tr>', //如果有fixed，则只要一次就可以找出来，否则需要三次
      !options.fixed ? '<% for(var i = 0; i < 3; i++){ %>' : '', '{{each item1 item2 i2}}', function () {
      ...
      '<th{{if item2.minWidth}} data-minWidth="{{item2.minWidth}}"{{/if}}{{if item2.unresize || item2.colspan>1}} unresize="{{item2.unresize}}"{{/if}}', '{{if item2.field}} data-field="{{item2.field}}"{{/if}}', ' data-key="{{item2.key}}" colspan="{{item2.colspan}}" rowspan="{{item2.rowspan}}"', ' class="{{if item2.sort && item2.colspan==1}}is-sortable{{/if}}{{if item2.type!=="normal"}} dui-table__cell-{{item2.type}}{{/if}}{{if item2.align}} is-{{item2.align}}{{/if}}">', '<div class="cell dui-table-{{if item2.colGroup}}group{{else}}{{index}}-{{item2.key}}{{/if}}">', '{{if item2.type=="checkbox"}}', '<input type="checkbox" dui-checkbox indeterminate="true"{{if checkAll}} checked="checked"{{/if}} dui-filter="checkAll">', '{{else}}', '{{item2.title||item2.field}}', '{{/if}}', // 排序
      '{{if item2.sort && item2.colspan==1 && item2.type!=="checkbox" && item2.type!=="numbers"}}', '<span class="caret-wrapper{{if initSort && initSort.field==item2.field && initSort.sort=="desc"}} descending{{/if}}{{if initSort && initSort.field==item2.field && initSort.sort && initSort.sort!=="desc"}} ascending{{/if}}">', '<i class="sort-caret ascending"></i>', '<i class="sort-caret descending"></i>', '</span>', '{{/if}}', // 
      ...
      ' style="{{if height}}height:{{height}}px{{/if}}">', '<div class="dui-table__box">', '<div class="dui-table__header-wrapper">', TMPL_HEAD(), '</div>', '<div class="dui-table__body-wrapper{{if initScroll}} {{initScroll}}{{/if}}" style="height:{{bodyHeight}}px;">', // 显示内容，暂不设置
      '</div>', '{{if fixedLCoumnsNum>0}}', '<div class="dui-table__fixed dui-table__fixed-left">', '<div class="dui-table__fixed-header-wrapper">', TMPL_HEAD({
        fixed: 'left'
      }), '</div>', '<div class="dui-table__fixed-body-wrapper" style="height:{{bodyHeight}}px;">', '</div>', '</div>', '{{/if}}', '{{if fixedRCoumnsNum>0}}', '<div class="dui-table__fixed dui-table__fixed-right">', '<div class="dui-table__fixed-header-wrapper">', TMPL_HEAD({
        fixed: 'right'
      }), '</div>', '<div class="dui-table__fixed-body-wrapper" style="height:{{bodyHeight}} px;">', '</div>', '</div>', '{{/if}}', // 补丁
      '<div class="dui-table__fixed-right-patch"></div>', '</div>', '{{if page.show}}', '<div class="dui-table__page">', '</div>', '{{/if}}', '{{if filterArr.length>0}}', '<div class="dui-table__condition">', '<div class="dui-table__condition-label">高级查询</div>', '<div class="dui-table__condition-body">', '<div class="dui-table__condition-body-inner">', '<div class="dui-table__condition-items"></div>', '</div>', '</div>', '</div>', '{{/if}}', // 设置样式
      '<style>', '{{each columns  item1 i1}}', '{{each item1 item2 i2}}', '{{if item2.initWidth}}', '.dui-table-{{index}}-{{item2.key}}{', 'width:{{item2.initWidth}}px;', '}', '{{/if}}', '{{/each}}', '{{/each}}', '</style>', '</div>'].join('');
    },
    ...
    thisTable = function thisTable() {
      ...
      return {
        ...
        hideFilterForm: function hideFilterForm() {
    ...
    Class = function Class(options) {
      ...
      config.el = $(config.el);
      config.original = $(config.el); // 如果没有原始元素

      if (!config.el[0]) return that; //高度铺满：full-差距值
      ...
      }, config.treeTable) : false; // 如果是treeTable则不显示分页
      ...
      that.currPage = parseInt(config.page.curr) || 1; // 设置初始化排序列

      config.initSort ? that.sortKey = $.extend(true, {}, config.initSort) : that.sortKey = {}; // 初始化
    ...
    Class.prototype.init = function () {
      ...
      var str = template.render(TMPL_MAIN(), config),
          ...
          duiLoading = that.duiLoading = popup.loading({
        target: reElem[0]
      }),
          duiCondition = that.duiCondition = reElem.find(CONDITION); // 如果有条件显示元素则初始化

      duiCondition[0] && that.initFilter(); // 如果已经渲染过了则移除

      hasRender[0] && hasRender.remove();
      el.after(reElem); // 设置表格大小样式

      that.fullSize(); // 同步header高度
      ...
      that.renderForm(); // 获取数据

      that.pullData(that.currPage); // 设置事件

      that.setEvent(); // 设置当前状态已经初始化完毕，因为可能会有ajax请求，所以确保ajax请求完毕
    ...
    Class.prototype.initFilter = function () {
      var theTable = this,
          theOption = theTable.config,
          dataId = 1,
          filterArr = theOption.filterArr,
          filterObj = theOption.filterObj; // 定义条件
      ...
      var filterForm = function filterForm() {
        ...
        that.events = {
          ...
          addCondition: function addCondition(e) {
            ...
            var itemDom = $(['<li class="dui-table__filter-item" data-id="' + dataId++ + '" data-prefix="and" data-field="' + frist.field + '" data-mod="condition" data-type="' + frist.type + '" data-condition="' + condition[frist.type][0] + '" data-value="">', '<div class="dui-table__filter-item-content">', '<div class="dui-table__filter-item-prefix">', '<input type="checkbox" dui-switch value="and" active-value="and" inactive-value="or" active-text="且" inactive-text="或" skin="label-in" width="43" inactive-color="#67c23a">', '</div>', '<div class="dui-table__filter-item-field">', '<label filter-event="fieldChange" class="dui-firebrick">' + frist.title + '</label>', '</div>', '<div class="dui-table__filter-item-condition">', '<label filter-event="conditionChange" class="dui-deeppink">' + Allcondition[condition[frist.type][0]] + '</label>', '</div>', '<div class="dui-table__filter-item-value">', '<label filter-event="valueChange" class="dui-blueviolet">' + (frist.type == 'enum' ? '请选择...' : '请输入...') + '</label>', '</div>', '<div class="dui-table__filter-item-delete">', '<label filter-event="deletItem"><i class="dui-icon-circle-close"></i></label>', '</div>', '</div>', '</li>'].join(''));

            itemDom.find('input[type="checkbox"]')[0].onchange = function (e) {
              var value = $(this).val();
              itemDom.attr('data-prefix', value); // 同步table表格底部条件

              theTable.sysFilterForm();
            }; // 渲染dom


            form.render(itemDom); // 添加元素

            ul.append(itemDom); // 同步table表格底部条件
          ...
          addGroup: function addGroup(e) {
            ...
            var itemDom = $(['<li class="dui-table__filter-item"  data-id="' + dataId++ + '" data-mod="group" data-prefix="and">', '<div class="dui-table__filter-item-content">', '<div class="dui-table__filter-item-prefix">', '<input type="checkbox" dui-switch value="and" active-value="and" inactive-value="or" active-text="且" inactive-text="或" skin="label-in" width="43" inactive-color="#67c23a">', '</div>', '<div class="dui-table__filter-item-field">', '<label class="dui-firebrick">分组</label>', '</div>', '<div class="dui-table__filter-item-operation">', '<div class="dui-button-group">', '<button class="dui-button dui-button--primary dui-button--mini" filter-event="addCondition">添加条件</button>', '<button class="dui-button dui-button--primary dui-button--mini" filter-event="addGroup">添加分组</button>', '</div>', '</div>', '<div class="dui-table__filter-item-delete">', '<label filter-event="deletItem"><i class="dui-icon-circle-close"></i></label>', '</div>', '</div>', '<ul class="dui-table__filter-item-group">', '</ul>', '</li>'].join(''));
          ...
          deletItem: function deletItem(e) {
            var othis = $(this),
                li = othis.parents(SELECTOR.itemContent).parent();
            li.remove(); // 同步table表格底部条件
          ...
          fieldChange: function fieldChange(e) {
            var othis = $(this),
                dataDom = othis.parents(SELECTOR.itemContent).parent('li');

            if (this.popverDom && $(this.popverDom).css('display') !== 'none') {
              this.hide();
            } else {
              var currField = dataDom.attr('data-field');
              var content = $(['<ul class="dui-select-dropdown__list">', function () {
                var res = '';
              ...
              content.on('click', '.dui-select-dropdown__item', function (e) {
                ...
                if (!$item.hasClass('selected')) {
                  ...
                  var fristConditionTitle = Allcondition[fristCondition]; // 设置当前选中

                  $item.addClass('selected').siblings().removeClass('selected'); // 设置dataDom属性
                  ...
                  dataDom.children().find(SELECTOR.itemField + '>label').text(ofieldTitle); // 设置condition的title

                  dataDom.children().find(SELECTOR.itemCondition + '>label').text(fristConditionTitle); // 设置值的显示

                  var value = $.inArray(otype, ['enum', 'timestamp']) != -1 ? '请选择...' : '请输入...';
                  dataDom.children().find(SELECTOR.itemValue + '>label').text(value); // 关闭弹窗

                  othis[0].hide(); // 同步table表格底部条件
          ...
          conditionChange: function conditionChange(e) {
            ...
            if (this.popverDom && $(this.popverDom).css('display') !== 'none') {
              ...
              content.on('click', '.dui-select-dropdown__item', function (e) {
                ...
                if (!$item.hasClass('selected')) {
                  var currCondition = $item.attr('dui-condition'); // 设置当前选中
                  ...
                  dataDom.children().find(SELECTOR.itemCondition + '>label').text(Allcondition[currCondition]); // 设置值的显示
          ...
          valueChange: function valueChange(e) {
            ...
            if (this.popverDom && $(this.popverDom).css('display') !== 'none') {
              ...
              var currType = dataDom.attr('data-type');
              var currCondition = dataDom.attr('data-condition');
              var currValue = dataDom.attr('data-value'); // 如果是枚举类型

              if (currType == 'enum' || currType == 'boolean') {
                // 获取枚举数据
                var options = currType == 'boolean' ? {
                  true: 'true',
                  false: 'false'
                } : filterObj[currField].options;
                var content = $(['<ul class="dui-select-dropdown__list">', function () {
                  var res = '';
                ...
                content.on('click', '.dui-select-dropdown__item', function (e) {
                  ...
                  if (!$item.hasClass('selected')) {
                    var value = $item.attr('dui-value'); // 设置当前选中

                    $item.addClass('selected').siblings().removeClass('selected'); // 修改dataDom属性

                    dataDom.attr('data-value', value); //设置值
                    // 设置值的显示

                    dataDom.children().find(SELECTOR.itemValue + '>label').text(options[value]); // 关闭弹窗
                ...
                that.events.showPopover(this, content[0]);
              } else if (currType == 'timestamp') ; else {
                // 如果是介于
                if (currCondition == 'between') {
                  var content = $(['<div class="dui-range-editor dui-input__inner dui-range-editor--mini">', '<input autocomplete="off" placeholder="开始值" name="" class="dui-range-input" value="' + function () {
                    ...
                    return '';
                  }() + '">', '<span class="dui-range-separator">到</span>', '<input autocomplete="off" placeholder="结束值" name="" class="dui-range-input" value="' + function () {
                    var arr = currValue.split(',');
                  ...
                  content.find('input').on('blur', function (e) {
                    ...
                    setTimeout(function () {
                      ...
                      if (!content.find('input').is(':focus')) {
                        ...
                        var value = [inputs.eq(0).val(), inputs.eq(1).val()]; // 修改dataDom属性
                  ...
                  var content = $(['<div class="dui-input dui-input--mini">', '<input type="text" class="dui-input__inner" placeholder="请输入..." value="' + currValue + '">', '</div>'].join(''));
                  othis.hide();
                  othis.after(content);
                  content.children('input').focus().on('blur', function (e) {
                    var value = $(this).val(); // 修改dataDom属性
          ...
          showPopover: function showPopover(ref, content) {
            var popverDom = $(['<div class="dui-popper dui-popver dui-table__popver" style="display:none">', '<div x-arrow="" class="popper__arrow"></div>', '</div>'].join(''));
      ...
      filterForm.prototype.show = function (options) {
        ...
        $(that.formDom).on('click', '[filter-event]', function (e) {
          var othis = $(this),
              oEnvent = othis.attr('filter-event');
    ...
    Class.prototype.sysFilterForm = function (genxin) {
      var that = this,
          options = that.config,
          filterFormIns = that.filterForm,
          formDom = filterFormIns.formDom,
          filterArr = options.filterArr,
        ...
        conditionItems: '.dui-table__condition-body-inner'
          ...
          getSysHtml = function getSysHtml(ul) {
        ...
        lis.each(function (i, li) {
          ...
          if ($li.attr('data-mod') == 'group') {
            ...
            if (temUl.children('li').length > 0) {
              res += ['<div class="dui-table__condition-item" data-id="' + id + '" data-prefix="' + prefix + '">', res != '' ? '<div class="dui-table__condition-prefix dui-red">' + prefixs[prefix] + '</div>' : '', getSysHtml(temUl), '</div>'].join('');
            }
          } else {
            res += ['<div class="dui-table__condition-item" data-id="' + id + '" data-prefix="' + prefix + '" data-field="' + field + '" ', 'data-mod="' + mod + '" data-type="' + type + '" data-condition="' + condition + '" data-value="' + value + '">', res != '' ? '<div class="dui-table__condition-prefix dui-red">' + prefixs[prefix] + '</div>' : '', '<div class="dui-table__condition-field dui-firebrick">' + filterObj[field].title + '</div>', '<div class="dui-table__condition-condition dui-deeppink">' + Allcondition[condition] + '</div>', '<div class="dui-table__condition-value dui-blueviolet">' + valueTitle + '</div>', '</div>'].join('');
          }
        });
        return res;
      },
          getSysData = function getSysData(ul) {
        var lis = ul.children('li');
        var res = [];
        lis.each(function (i, li) {
          var $li = $(li),
      ...
      var shishi = formDom.find(SELECTOR.toolbar).find('#auto_search').is(':checked'); // 判断是否实时跟新

      if (shishi || genxin) {
        var ul = formDom.find(SELECTOR.fristUl); // 则同步腾讯

        var template = getSysHtml(ul);
        var filterData = that.filterData = getSysData(ul); // 设置显示信息

        $(that.reElem).find(SELECTOR.conditionItems).children('div').html(template); // 拉取数据
    ...
    Class.prototype.initColumns = function () {
      var that = this,
          ...
          filterObj = {},
          filterArr = [],
          initColWidthWithType = function initColWidthWithType(item) {
        var initWidth = {
          checkbox: 48,
          space: 15,
          numbers: 40
      ...
      config.filterObj = filterObj, config.filterArr = filterArr; // 初始化列宽度
    ...
    Class.prototype.fullSize = function () {
      ...
      if (!height) return;
      bodyHeight = parseFloat(height) - (that.duiHeader.outerHeight() || 48); //减去分页栏的高度
    ...
    Class.prototype.setFixedStyle = function () {
      var that = this,
          bodyHeight = that.duiBodyer.height(),
          ...
          headerHeight = that.duiHeader.height(); // 设置浮动的高要考虑1px的boarder

      that.duiFixed.css('height', that.fixedHeight - (scrollHeight ? scrollHeight : 1)); // 设置左侧浮动的宽

      that.duiFixedL.css('width', fixedLWidth); // 设置左侧浮动的宽

      that.duiFixedR.css('width', fixedRWidth); // 设置浮动内容的高度和距离header的高度
    ...
    Class.prototype.initColumnsWidth = function () {
      ...
      ,
          ...
          fixedLCoumnsNum = options.fixedLCoumnsNum = 0,
          //左侧浮动列个数
      fixedRCoumnsNum = options.fixedRCoumnsNum = 0,
          //右侧浮动列个数
      ...
      var getAutoWidth = function getAutoWidth(back) {
        that.eachColumns(function (i, col) {
          var width = 0,
          ...
          if (col.hide) width = 0;
          countWidth = countWidth + width; //如果未填充满，则将剩余宽度平分

          clientWidth > countWidth && autoColNums && (autoWidth = (clientWidth - countWidth) / autoColNums);
        }, true);
      };

      getAutoWidth();
      getAutoWidth(true); // 设置列宽
      ...
      that.eachColumns(function (index, col) {
        ...
        if (col.width === 0) {
          col.initWidth = Math.floor(autoWidth >= minWidth ? autoWidth : minWidth); // 设置当前列为自动分配宽度列
    ...
    Class.prototype.resColumnsWidth = function () {
      ...
      }, true); //填补 Math.floor 造成的数差

      var patchNums = that.duiBodyer.width() - that.getScrollWidth(that.duiBodyer[0]) - that.duiBodyer.children('table').outerWidth();

      if (autoColNums && patchNums >= -allCols && patchNums <= allCols) {
        var getEndTh = function getEndTh(th) {
          var field;
        ...
        that.getCssRule(key, function (item) {
          ...
          item.style.width = parseFloat(width) + patchNums + 'px'; //二次校验，如果仍然出现横向滚动条（通常是 1px 的误差导致）
    ...
    Class.prototype.setScrollPatch = function () {
      ...
      Surplus = that.duiBodyer.prop('offsetWidth') - duiMainTable.outerWidth(),
          patchWidth = Surplus > scrollWidth ? Surplus : scrollWidth; // 如果有右侧滚动条

      if (scrollWidth > 0) {
        that.duiHeader.find('thead>tr').eq(0).append(that.duiGutter); // 如果是多级表头
    ...
    Class.prototype.eachColumns = function (callback, oldItem) {
      ...
      var eachArrs = function eachArrs(obj) {
        $.each(obj || arrs, function (i, item) {
          ...
          getOldItem = function getOldItem(item) {
        var res;
    ...
    Class.prototype.pullData = function (curr) {
      ...
      that.startTime = new Date().getTime(); //如果需要显示加载条

      options.loading && that.duiLoading.show(); // 如果是数组
      ...
      if (ajaxOpt.url) {
        ...
        if (ajaxOpt.contentType && ajaxOpt.contentType.indexOf("application/json") == 0) {
          //提交 json 格式
        ...
        $.ajax({
          ...
          success: function success(res) {
            if (res[response.statusName] === response.statusCode) {
              ...
              that.duiBodyer.html(template.render(TMPL_TIP, {
                text: options.text.loadError
              }));
            }
          },
        ...
        that.duiBodyer.html(template.render(TMPL_TIP, {
          text: options.text.notDataOrUrl
        }));
      }
    };
    /**
    ...
    Class.prototype.renderData = function (res, curr, count) {
      ...
      setBody = function setBody() {
        ...
        '{{@ duiIndex}}', '{{else}}', function () {
          if (options.treeTable) {
            return ['{{if field=="' + options.treeTable.expandColumn + '"}}', '<span class="dui-table__row--level_#{#level#}#" style="padding-left:#{#(level-1)*16#}#px">', '#{#if hasChild#}#', '<span class="dui-table__expand-icon#{#if ' + options.treeTable.expandAll + ' || expand #}# dui-table__expand-icon--expanded#{#/if#}#">', '<i class="' + (options.treeTable.expandIcon ? options.treeTable.expandIcon : ' dui-icon-arrow-right') + '"></i>', '</span>', '#{#else#}#', '<span class="dui-table__placeholder"></span>', '#{#/if#}#', '</span>', '{{/if}}'].join('');
          }
        }(), '{{@ fieldName}}', '{{/if}}', '{{/if}}', '</div>', '</td>'].join('');
        ...
        tds = tds_lef.concat(tds).concat(tds_right); // 循环数据
        ...
        $.each(resData, function (index, row) {
          ...
          var tr_left = tds_lef.length > 0 ? $('<tr >' + template.render(tds_lef.join(''), row) + '</tr>') : '',
              ...
              tr = tds.length > 0 ? $('<tr >' + template.render(tds.join(''), row) + '</tr>') : ''; // 设置数据

          tr_left[0] ? tr_left[0].data = row : '';
          tr_right[0] ? tr_right[0].data = row : '';
          tr[0] ? tr[0].data = row : ''; // 如果是树形表格则判断是否显示该列
        ...
        that.duiFixedRWrap.html(''), that.duiFixedRWrap.append(bodyRightTable); // 渲染form

        that.renderForm(); // 重新设置浮动高度

        that.fullSize(); // 重新设置列宽

        that.resColumnsWidth(); // 重新设置浮动样式

        that.setFixedStyle(); // 设置补丁
    ...
    Class.prototype.setEvent = function () {
      ...
      th.on('mousemove', function (e) {
        var othis = $(this),
            ...
            pLeft = e.clientX - oLeft; // 不允许拖拽

        if (othis.attr('unresize') === 'true' || dict.resizeStart) return; // 是否处于拖拽允许区域

        dict.allowResize = othis.width() - pLeft <= 10; // 如果处于允许移动范围则修改指针样式
        ...
        if (dict.allowResize) {
          ...
          e.preventDefault(); // 开始拖拽

          dict.resizeStart = true; // 记录初始坐标

          dict.offset = [e.clientX, e.clientY]; // 获取当前列的样式规则
    ...
    Class.prototype.sort = function (field, type) {
      var that = this,
          ...
          ClassName;
      if (th.length == 0) return; // 如果已经在状态中

      if (that.sortKey.field == field && that.sortKey.sort == type) return; // 如果当前选中字段和已经有的字段不一致
      ...
      th.siblings().find(HEADER_SORT).attr('class', 'caret-wrapper'); // 设置当前样式
      ...
      th.find(HEADER_SORT).attr('class', ClassName); // 执行排序请求
    ...
    Class.prototype.synScroll = function () {
      ...
      that.duiFixed.find(FIXED_WRAP).scrollTop(scrollTop); // 渲染shown
    ...
    Class.prototype.buildRequest = function () {
      ...
      sort && (params[request.sortName] = sort); // 如果有where

      where && (params[request.whereName] = where); // 如果有filterData
